...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%Prob0.2.10%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   \item[0.2.10]  Prove for any given positive integer $N$ there exist only
      ...
      is nonempty. Let $m \in S_N$. To avoid the case where $m = 1$, we  further
      asume that $N > 1$. Note that $S_1 = \{1\}$ is finite. By the Fundamental 
      ...
      positive integers. Applying $\varphi$ to $m$ gives us 
      \begin{align}
      ...
      Now if one of the $p_i$s is greater than $N + 1$, then \eqref{0_2_1}
      says that $\varphi(m) > N$, a contradiction since $\varphi(m) = N$. So 
      each $p_i \le N + 1$. Similarly, since ${p_i}^{N} > N$, it must be the
      case that each $c_i < N + 1$. Now let $T = \{q_1, q_2, \ldots, q_t\}$ be 
      the set of all primes not greater than $N + 1$, where
      $q_1 < q_2 < \cdots < q_t$. It follows that each $p_i \in T$ and thus
      $s \le t$; that is,
      $$m = {p_1}^{c_1}{p_2}^{c_2}\cdots{p_s}^{c_s} <
        \underbrace{{q_t}^{N+1}{q_t}^{N+1}\cdots{q_t}^{N+1}}_{t \text{ times}} 
        = {q_t}^{(N+1)t}.$$
      Above, we used the fact that every element of $S_N$ can have at most
      $t$ primes in its factorization. Thus every element of $S_N$ is less than
      ${q_t}^{(N+1)t}$ and it follows that
      $$S_N \subsetneq \{1, 2, \ldots, {q_t}^{N+1}\},$$ 
      ...
      $\varepsilon$). Consider $n > K$ and assume that
