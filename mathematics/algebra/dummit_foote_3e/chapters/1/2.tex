...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%1.2.4%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   \item[1.2.4]   If $n = 2k$ is even and $n \ge 4$, show that $z = r^k$ is an
      ...
      $r^kr^k = 1$, it follows that $r^{-k} = r^k$. Now consider $w \in D_{2n}$;
      if $w$ is an integral power of $r$, then $r^k$ commutes with $w$, so
      assume that $w = sr^i$, with $0 \le i < n$. Then we have that
      \begin{align*}
         zw &= r^k(sr^i)  \\
            &= (r^ks)r^i &[\text{Associativity}] \\
               &= (sr^{-k})r^i  &[\text{Lemma 1.2.1}] \\
               &= (sr^k)r^i     &[r^k = r^{-k}] \\
               &= s(r^kr^i)     &[\text{Associativity}] \\
               &= s(r^ir^k)     &[\text{Exercise 1.1.19}] \\
               &= (sr^i)r^k = wz. &[\text{Associativity}]
      \end{align*}
